...
import { Clock, Film, Calendar, Play, Ticket, ChevronRight, MapPin, X } from 'lucide-react';
...
export default function MovieDetailPage() {
  ...
  const { selectedCinemaId, selectedCinemaName, setSelectedCinema, clearSelectedCinema } = useLocationStore();
  ...
  const handleCinemaSelect = (cinemaId: string, cinemaName: string) => {
    ...
    toast.success(`Bioskop dipilih: ${cinemaName}`);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      {/* ─── Booking Section (hidden until Beli Tiket clicked) ────── */}
      {showBooking && (
        <div id="booking-section" className="max-w-6xl mx-auto px-6 py-10">
          <div className="bg-gray-900/60 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-gray-500 text-white px-4 py-2.5 rounded-xl transition-all text-sm"
              >
                <MapPin className="w-4 h-4 text-red-400" />
                {selectedCinemaName ? (
                  <span className="font-medium">{selectedCinemaName}</span>
                ) : (
                  <span className="text-gray-400">Pilih Bioskop</span>
                )}
            ...
            {selectedCinemaName && (
              <div className="flex items-center gap-2 mb-5 bg-red-600/10 border border-red-600/30 rounded-xl px-4 py-2.5">
                <MapPin className="w-4 h-4 text-red-400 flex-shrink-0" />
                <span className="text-red-300 text-sm font-medium flex-1">
                  Menampilkan jadwal di: {selectedCinemaName}
                </span>
                <button
                  onClick={() => clearSelectedCinema()}
                  className="text-gray-400 hover:text-white transition-colors"
                  title="Tampilkan semua bioskop"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            {loadingSchedules ? (
                ...
                {[1, 2].map((i) => (
                  <div key={i} className="bg-gray-800/50 rounded-xl p-4 animate-pulse">
                    ...
                    <div className="flex gap-2">
                      {[1, 2, 3].map((j) => (
                        <div key={j} className="h-14 w-24 bg-gray-700 rounded-lg" />
                      ))}
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="font-medium">Tidak ada jadwal untuk tanggal ini</p>
                {selectedCinemaId && (
                  <p className="text-sm mt-2">
                    di {selectedCinemaName}.{' '}
                    <button
                      onClick={clearSelectedCinema}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {group.schedules.map((schedule) => (
                        <button
                          ...
                          className={`group relative px-5 py-3 rounded-xl border transition-all text-left ${
                            schedule.isSoldOut
                              ? 'border-gray-700 text-gray-600 cursor-not-allowed bg-gray-800/30'
                              : 'border-white/15 bg-gray-800/80 text-white hover:border-red-500/60 hover:bg-red-600/10 active:scale-95'
                          }`}
                        >
          ...
          {/* Synopsis */}
          <div className="mt-6 bg-gray-900/40 border border-white/5 rounded-2xl p-6">
      ...
      {/* Synopsis (shown saat booking section belum muncul) */}
      {!showBooking && (
